...
def main():
  parser = argparse.ArgumentParser(
      description="Entrenamiento Pure CNN (5 Clases + Class Weights)"
  )
  parser.add_argument(
  ...
  print("=== PURE CNN2D BASELINE (5 CLASES CON PESOS BALANCEADOS) ===")
  print(
  ...
  logger = ExperimentLogger(
      experiment_name=f"Pure_CNN_5Class_Weighted_Seed_{args.seed}",
      base_dir=PROJECT_DIR / "runs",
  )
  ...
  print(f"⚖️ Pesos de {num_classes} clases calculados: {class_weights}")
  ...
  best_model_path = (
      CHECKPOINT_DIR / f"pure_cnn_5class_weighted_seed_{args.seed}_best.pth"
  )
  ...
  for epoch in range(1, EPOCHS + 1):
    ...
    print(
        f"Época [{epoch:02d}/{EPOCHS:02d}] | Train Loss: {tr_loss:.4f} - Acc:"
        f" {tr_acc:.2f}% | Val Loss: {val_loss:.4f} - Acc: {val_acc:.2f}%"
    )
