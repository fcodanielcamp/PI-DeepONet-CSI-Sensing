...
def build_dataset():
    ...
    # 🚀 CORRECCIÓN: la fase NO se normaliza con Z-score.
    # Es una variable circular acotada a [-pi, pi] por construcción; restarle
    # una media empírica y reescalar por una desviación estándar distorsiona
    # su significado físico y rompe la consistencia de H_obs_complex,
    # h_target (CIR) y D_target (Doppler) calculados aguas abajo en
    # compute_pi_loss. Se mantiene en radianes crudos.
    mean_phase = 0.0
    std_phase = 1.0

    print(f"\nEstadísticas Z-Score obtenidas strictly del conjunto TRAIN:")
    print(f"  Amplitud: Mean = {mean_amp:.4f}, Std = {std_amp:.4f}")
    print(f"  Fase: sin normalizar (se conserva en radianes crudos, rango [-pi, pi])")
    ...
    for i in range(0, tr_shape[0], chunk_size):
        ...
        # 🚀 CORRECCIÓN: canal 1 (fase) se deja intacto, sin normalizar.
