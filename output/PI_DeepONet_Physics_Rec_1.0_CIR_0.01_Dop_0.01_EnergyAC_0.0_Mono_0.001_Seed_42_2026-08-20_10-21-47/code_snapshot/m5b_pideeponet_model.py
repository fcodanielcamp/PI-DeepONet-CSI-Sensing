...
class BranchNet(nn.Module):
  ...
  def __init__(self, latent_dim=128):
    ...
    # 🚀 CORRECCIÓN: pooling asimétrico progresivo.
    # pool1 y pool2 NO reducen el eje temporal (kernel=(1,2)): se preserva
    # resolución temporal completa (25 frames) durante las dos primeras
    # convoluciones, donde el kernel (3,5) puede extraer estructura fina
    # frame-a-frame relevante para el término Doppler de compute_pi_loss.
    # pool3 comprime ambos ejes (kernel=(2,2)) solo al final.
    # Dimensiones resultantes: tiempo 25->25->25->12 ; frecuencia 241->120->60->30
    self.pool1 = nn.MaxPool2d(kernel_size=(1, 2))
    self.pool2 = nn.MaxPool2d(kernel_size=(1, 2))
    self.pool3 = nn.MaxPool2d(kernel_size=(2, 2))
    self.act = nn.Mish()
    self.fc = nn.Linear(64 * 12 * 30, latent_dim)
    # 🚀 CORRECCIÓN: LayerNorm tras fc. El Diagnóstico C mostró que b_out
    # (salida de fc, sin normalizar hasta ahora) amplifica el desplazamiento
    # de dominio medido en las capas convolucionales (|z_shift| medio 0.05-0.11
    # con BatchNorm -> 0.304, máximo 0.966, en b_out). A diferencia de
    # BatchNorm, LayerNorm normaliza cada muestra individualmente usando solo
    # sus propias 128 dimensiones -- no depende de estadísticas acumuladas de
    # ningún dominio específico, por lo que no puede sufrir el mismo tipo de
    # desajuste train/test. Al ser b_out el origen compartido del clasificador,
    # el decoder y loss_energy, esta corrección se propaga a los tres.
...
def compute_pi_loss(
  ...
  # 3. Pérdida de Energía Dispersada -- ahora basada en energía Doppler AC
  # (técnica MTI: Moving Target Indication), no en la norma total del campo
  # latente. D_hat tiene forma (B, 25, 241): el eje de tamaño 25 es el eje
  # Doppler resultante de la FFT temporal. El bin DC (índice 0 de ese eje)
  # corresponde a dispersores ESTÁTICOS -- geometría propia de cada sala,
  # paredes, muebles --; los bins restantes (AC) corresponden
  # específicamente a dispersores EN MOVIMIENTO (personas). A diferencia de
  # la norma del campo latente completo (que mezclaba ambas fuentes y se
  # confirmó contaminada por la geometría de la sala en el diagnóstico de
  # b_out: latent_norm de "0 personas" en test superaba a "3 personas" en
  # train), esta energía descarta explícitamente la parte específica de
  # dominio antes de compararla con el número de personas.
  D_hat_ac = D_hat[:, 1:, :]  # excluye el bin DC (índice 0 del eje Doppler)
  ac_energy_norm = torch.sqrt(
      torch.sum(torch.abs(D_hat_ac) ** 2, dim=(1, 2)) + 1e-8
  )
  target_energy = gamma * y_people.float()
  loss_energy = torch.mean((ac_energy_norm - target_energy) ** 2)

  # latent_norms se conserva sin cambios: loss_mono sigue operando sobre el
  # campo latente completo, no sobre la energía AC.
