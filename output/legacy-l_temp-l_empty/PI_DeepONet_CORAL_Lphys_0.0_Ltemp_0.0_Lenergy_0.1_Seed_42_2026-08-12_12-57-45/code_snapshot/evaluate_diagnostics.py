...
def _evaluate_split(model, loader, model_type, num_classes):
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            # 🚀 Detección automática: ¿Es CORAL (K-1) o Estándar (K)?
            is_coral = (logits.size(1) == num_classes - 1)

            if is_coral:
                # 1. Predicción ordinal CORAL (conteo de umbrales)
                cum_probs = torch.sigmoid(logits)
                preds = torch.sum(cum_probs > 0.5, dim=1)

                # 2. Reconstrucción de probabilidades de clase independientes (para curva ROC)
                batch_size = logits.size(0)
                probs = torch.zeros(batch_size, num_classes, device=DEVICE)
                
                probs[:, 0] = 1.0 - cum_probs[:, 0]  # P(y=0)
                for k in range(1, num_classes - 1):
                    probs[:, k] = cum_probs[:, k-1] - cum_probs[:, k]  # P(y=k)
                probs[:, -1] = cum_probs[:, -1]  # P(y=K-1)
                
                # Prevenir inestabilidad numérica
                probs = torch.clamp(probs, min=0.0, max=1.0)
            else:
                # Clasificación Multiclase Estándar
...
def run_post_hoc_diagnostics(model_path, model_type="pideeponet", output_dir=None, run_dir=None, num_classes=3):
    ...
    # Resolución del directorio de salida en output/
    ...
    for idx, (split_name, dataset) in enumerate(datasets.items()):
        ...
        # 🚀 Se pasa num_classes a _evaluate_split para la reconstrucción CORAL
        y_true, y_pred, y_probs = _evaluate_split(model, loader, model_type, num_classes)
