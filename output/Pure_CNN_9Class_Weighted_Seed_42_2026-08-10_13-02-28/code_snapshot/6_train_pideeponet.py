...
def main():
    parser = argparse.ArgumentParser(description="Entrenamiento PI-DeepONet (9 Clases + Class Weights)")
    parser.add_argument("--seed", type=int, default=42, help="Semilla aleatoria")
    args = parser.parse_args()

    set_seed(args.seed)

    print("=== MÓDULO 6: PI-DEEPONET (9 CLASES CON PESOS BALANCEADOS) ===")
    print(f"Dispositivo activo: {DEVICE} ({torch.cuda.get_device_name(0)}) | Semilla: {args.seed}")

    logger = ExperimentLogger(experiment_name=f"PI_DeepONet_9Class_Weighted_Seed_{args.seed}", base_dir=PROJECT_DIR / "runs")
    ...
    best_model_path = CHECKPOINT_DIR / f"pideeponet_9class_weighted_seed_{args.seed}_best.pth"
