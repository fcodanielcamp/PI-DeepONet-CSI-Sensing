...
# ==============================================================================
# 2. ARQUITECTURA PI-DEEPONET (CON CABEZA ORDINAL CORAL)
# ==============================================================================
...
class PIDeepONet(nn.Module):
    def __init__(self, num_classes=9, latent_dim=128):
        ...
        # Proyección escalar única para CORAL (sin sesgo en la última capa lineal)
        ...
        # K-1 umbrales ordinales (sesgos aprendibles para las tareas binarias)[cite: 2]
    ...
    def forward(self, x_branch):
        ...
        # Obtener proyección escalar y sumar los sesgos ordinales de CORAL
        projected = self.classifier(field_summary)
        logits = projected + self.coral_bias  # Forma resultante: (Batch, num_classes - 1)
        
        return logits, latent_field

# ==============================================================================
# 3. FUNCIÓN DE LOSS TOTAL (CORAL + Dirichlet + Temporal + Energía RCS)
# ==============================================================================
def compute_pi_loss(logits, latent_field, y_people, y_empty, lambda_phys=0.01, class_weights=None, lambda_temp=0.001, lambda_energy=0.0, gamma=1.0):
    # 1. Pérdida de datos ordinal CORAL (Binary Cross-Entropy acumulativa)[cite: 2]
    ...
    # 2. Pérdida de estado vacío (Condición de frontera Dirichlet)
    ...
    # 3. Pérdida de regularización temporal sobre el latent_field (Diferencias Finitas)
    batch_size = latent_field.size(0)
    latent_3d = latent_field.view(batch_size, 25, 241)
    dt = latent_3d[:, 1:, :] - latent_3d[:, :-1, :]
    loss_temp = torch.mean(dt ** 2)

    # 4. 🚀 Nueva Pérdida Física: Proporcionalidad de Energía Dispersada (RCS)
    # Calcula la norma L2 por muestra en el batch y la compara con la cantidad de personas escalada por gamma
    ...
    # Combinación ponderada total incluyendo la nueva pérdida de energía
    loss_total = loss_data + (lambda_phys * loss_phys) + (lambda_temp * loss_temp) + (lambda_energy * loss_energy)

    return loss_total, loss_data, loss_phys, loss_temp, loss_energy

if __name__ == "__main__":
    print(f"=== VERIFICACIÓN DE ARQUITECTURA PI-DEEPONET (CORAL + RCS ENERGY) ===")
    ...
    if not META_PATH.exists():
        ...
        loss_total, loss_data, loss_phys, loss_temp, loss_energy = compute_pi_loss(
            logits, latent_field, dummy_y_people, dummy_y_empty, lambda_energy=0.001
        )

        print(f"Entrada Branch: {dummy_x.shape} | Logits CORAL: {logits.shape} | Latent Field: {latent_field.shape}")
        print(f"Loss Total: {loss_total.item():.4f} (Data: {loss_data.item():.4f}, Phys: {loss_phys.item():.4f}, Temp: {loss_temp.item():.4f}, Energy: {loss_energy.item():.4f})")
        print("¡Prueba de arquitectura CORAL + Energía exitosa!")
