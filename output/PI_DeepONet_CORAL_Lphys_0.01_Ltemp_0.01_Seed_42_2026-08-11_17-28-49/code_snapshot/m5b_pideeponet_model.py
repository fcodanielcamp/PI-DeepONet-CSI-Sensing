...
# ==============================================================================
# 3. FUNCIÓN DE LOSS TOTAL (CORAL Ordinal + Física + Temporal)
# ==============================================================================
def compute_pi_loss(logits, latent_field, y_people, y_empty, lambda_phys=0.01, class_weights=None, lambda_temp=0.001):
    ...
    # Vectorizar etiquetas escalares a niveles binarios[cite: 2] (ej. y=3 -> [1, 1, 1, 0, 0, 0, 0, 0])
    ...
    # Combinación ponderada total
    loss_total = loss_data + lambda_phys * loss_phys + lambda_temp * loss_temp

    return loss_total, loss_data, loss_phys, loss_temp

if __name__ == "__main__":
    print(f"=== VERIFICACIÓN DE ARQUITECTURA PI-DEEPONET (CORAL) ===")
    ...
    if not META_PATH.exists():
        ...
        loss_total, loss_data, loss_phys, loss_temp = compute_pi_loss(logits, latent_field, dummy_y_people, dummy_y_empty)

        print(f"Entrada Branch: {dummy_x.shape} | Logits CORAL: {logits.shape} | Latent Field: {latent_field.shape}")
        print(f"Loss Total: {loss_total.item():.4f} (Data Ordinal: {loss_data.item():.4f}, Phys: {loss_phys.item():.4f}, Temp: {loss_temp.item():.4f})")
        print("¡Prueba de arquitectura CORAL exitosa!")
