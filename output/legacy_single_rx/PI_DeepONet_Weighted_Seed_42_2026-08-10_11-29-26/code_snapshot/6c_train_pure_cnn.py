...
def main():
    parser = argparse.ArgumentParser(description="Entrenamiento Pure CNN (3 Clases + Class Weights)")
    parser.add_argument("--seed", type=int, default=42, help="Semilla aleatoria")
    args = parser.parse_args()

    set_seed(args.seed)

    print("=== PURE CNN2D BASELINE (3 CLASES CON PESOS BALANCEADOS) ===")
    print(f"Dispositivo activo: {DEVICE} ({torch.cuda.get_device_name(0)}) | Semilla: {args.seed}")

    logger = ExperimentLogger(experiment_name=f"Pure_CNN_Weighted_Seed_{args.seed}", base_dir=PROJECT_DIR / "runs")
    ...
    best_model_path = CHECKPOINT_DIR / f"pure_cnn_weighted_seed_{args.seed}_best.pth"

    print("\nIniciando entrenamiento con pesos ponderados...")
    ...
    run_post_hoc_diagnostics(best_model_path, model_type="cnn", output_dir=logger.output_dir, num_classes=3)
