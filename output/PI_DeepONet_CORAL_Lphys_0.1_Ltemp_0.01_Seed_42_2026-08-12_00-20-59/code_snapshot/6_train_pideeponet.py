...
def train_epoch(model, dataloader, optimizer, device, lambda_phys, lambda_temp, class_weights_tensor):
    ...
    for x_b, y_people, y_empty in dataloader:
        ...
        # Cómputo de pérdida con regularización temporal y CORAL ordinal
...
def main():
    parser = argparse.ArgumentParser(description="Entrenamiento PI-DeepONet CORAL (9 Clases Ordinales)")
    ...
    print("=== MÓDULO 6: PI-DEEPONET CORAL (9 CLASES ORDINALES + LOSS TEMPORAL) ===")
    ...
    exp_name = f"PI_DeepONet_CORAL_Lphys_{args.lambda_phys}_Ltemp_{args.lambda_temp}_Seed_{args.seed}"
    ...
    best_model_path = CHECKPOINT_DIR / f"pideeponet_coral_lphys_{args.lambda_phys}_ltemp_{args.lambda_temp}_seed_{args.seed}_best.pth"

    print("Iniciando entrenamiento CORAL (Ordinal + Física + Temporal)...")
    ...
    print("\nCargando mejor modelo CORAL para evaluación Cross-Domain...")
